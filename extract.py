...
idor_search = ['manipulation', 'response', 'traversal', 'lfi', 'local file inclusion', 'business logic flaw', 'pollution', 'inclusion', 'reference', 'insecure direct object reference', 'spoofing', 'authorization']
broken_authentication_search = ['password', 'policy', 'session', 'httponly', 'cookie', 'mfa', 'rotate', 'flag', 'authentication', 'rotated', 'recaptcha', 'anti-automation', 'authenticate', 'pci-sensitive', 'broken', 'locking', 'lockout', 'bruteforce', 'rate limiting', 'brute force', 'brute-force', 'ticket', 'enumration', 'autocomplete']
...
for rows in range(number_of_rows):
    for columns in range(number_of_columns):
        if any (x in str(sheet.cell(rows,columns)).lower() for x in sql_injection_search):
            ...
            remote_injection_key.append(sheet.cell(rows,1))
            no_remote_injection_issues +=1
            remote_injection_file = open('rce.txt','a+')
            remote_injection_file.write(str(sheet.cell(rows,1))+'\n')
            continue
            ...
            xml_key.append(sheet.cell(rows,1))
            no_xml_issues +=1
            xml_file = open('xml.txt','a+')
            xml_file.write(str(sheet.cell(rows,1))+'\n')
            continue
